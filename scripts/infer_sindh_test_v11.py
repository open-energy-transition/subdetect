"""sindh_test prob rasters for v11_s2only: arm alone + mean fusion with v5 S1.

Same windowing as infer_sindh_test_arms.py / osmose_detect.py gated_inference.
Writes:
  sindh_test_v11/prob         v11 S2 arm alone       (run-name v11_s2only)
  sindh_test_v11mean/prob     (p_s1v5 + p_s2v11) / 2  (run-name v11_mean_fusion)

Usage:
  pixi run -e ml python scripts/infer_sindh_test_v11.py
"""
...
log = logging.getLogger("infer_sindh_test_v11")

S1_V5 = ROOT / "data/models/stageA_v5_s1only/terramind-sub-epoch=24-step=6975.ckpt"
S2_V11 = ROOT / "data/models/stageA_v11_s2only/terramind-sub-epoch=57-step=121394.ckpt"
WIN, STRIDE = 224, 104

REGION = ROOT / "data/osmose_regions/sindh_test"
OUT = {"v11s2": ROOT / "data/osmose_regions/sindh_test_v11/prob",
       "v11fused": ROOT / "data/osmose_regions/sindh_test_v11mean/prob"}
...
def main() -> None:
    ...
    tasks = {}
    for name, ckpt in (("s1v5", S1_V5), ("s2v11", S2_V11)):
        ...
        with rasterio.open(s2_path) as src, rasterio.open(s1_path) as src1:
            ...
            for r in rows:
                for c in cols:
                    ...
                    p2 = predict(*tasks["s2v11"], s2_np, None)
                    combo = {"v11s2": p2, "v11fused": (p1 + p2) / 2}
